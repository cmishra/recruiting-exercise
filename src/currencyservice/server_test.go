	...
	if len(target) == 1 && base == "USD" {
		...
		if target[0] == "" {
			expectedBody =
				...
				`"CHF":0.96326,"CNY":6.4845,"CZK":23.711,"DKK":6.5281,"GBP":0.68425,` +
				`"HKD":7.7581,"HRK":6.5869,"HUF":273.81,"IDR":13185.0,"ILS":3.7416,` +
...
func TestMultipleTargets(t *testing.T) {
	req, err := http.NewRequest("GET", "/rates?target=CAD&target=INR&timestamp=2016-04-29T14:34:46Z", nil)
	RequestCheck(t, err)
...
func SameDaySameResult(t *testing.T) {
	...
	BodyCheck(rr2, t, expectedBody)
}

func FutureDay(t *testing.T) {
